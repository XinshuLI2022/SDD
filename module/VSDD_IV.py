...
class VSDD(object):
    ...
    def __init__(self, x, v, t, y_ , p_t,s_mode1,s_mode2,FLAGS, r_alpha, r_beta, r_gamma, r_lambda,do_in, do_out, dims,pi_0=None):
        self.variables = {}
        self.wd_loss = 0
        ...
        self._build_graph(x, v, t, y_ , p_t, s_mode1,s_mode2,FLAGS, r_alpha, r_beta, r_gamma, r_lambda, do_in, do_out, dims,pi_0)
    ...
    def _build_graph(self, x, v,t, y_ , p_t, s_mode1,s_mode2,FLAGS, r_alpha, r_beta, r_gamma, r_lambda,do_in, do_out, dims,pi_0):
        ...
        with tf.name_scope("distangle"):
            ...
            h_rep_V, h_rep_norm_V, weights_in_V, biases_in_V = self._build_latent_graph(self.v,dim_v, dim_in, dim_out, FLAGS)
            h_rep_T, h_rep_norm_T, weights_in_T, biases_in_T = self._build_latent_graph(tf.concat((h_rep_norm_V,h_rep_norm_Z, h_rep_norm_C), axis=1),dim_in*3, dim_in, dim_out, FLAGS)
        ...
        weights_in = weights_in_Z + weights_in_C + weights_in_A +weights_in_T+weights_in_Y + weights_in_V
        biases_in = biases_in_Z + biases_in_C + biases_in_A + biases_in_T+ biases_in_Y + biases_in_V
        ...
        self.weights_in_V = weights_in_V
...
def trainNet(Net, sess, train_first, train_data, val_data, test_data, FLAGS, logfile, _logfile, exp):
    ...
    dict_factual = {Net.x: train_data['x'], Net.v: train_data['v'],Net.t: train_data['t'], Net.y_: train_data['yf'], \
    ...
    dict_valid = {Net.x: val_data['x'], Net.v: val_data['v'],Net.t: val_data['t'], Net.y_: val_data['yf'], \
            Net.s_mode1: 1., Net.s_mode2: 0., Net.do_in: 1.0, Net.do_out: 1.0, Net.r_alpha: FLAGS.p_alpha, \
            Net.r_beta: FLAGS.p_beta,Net.r_gamma: FLAGS.p_gamma, \
            Net.r_lambda: FLAGS.p_lambda, Net.p_t: p_treated}
    ...
    for i in range(FLAGS.iterations):
        ...
        v_batch = train_data['v'][I,:]
        t_batch = train_data['t'][I]
        y_batch = train_data['yf'][I]
    
      
        if not objnan:
            sess.run(train_first, feed_dict={Net.x: x_batch,Net.v: v_batch, Net.t: t_batch, \
        ...
        if i % FLAGS.output_delay == 0 or i==FLAGS.iterations-1:
            ...
            y_pred_f = sess.run(Net.output, feed_dict={Net.x: train_data['x'], Net.v: train_data['v'], \
                Net.t: train_data['t'], Net.s_mode1: 1., Net.s_mode2: 0., Net.do_in: 1.0, Net.do_out: 1.0})
            y_pred_cf = sess.run(Net.output, feed_dict={Net.x: train_data['x'], Net.v: train_data['v'],\
                Net.t: 1-train_data['t'], Net.s_mode1: 0., Net.s_mode2: 1., Net.do_in: 1.0, Net.do_out: 1.0})
            y_pred_mu0 = sess.run(Net.output, feed_dict={\
                Net.x: train_data['x'], Net.v: train_data['v'],Net.t: train_data['t']-train_data['t'], Net.s_mode1: 0., Net.s_mode2: 0., Net.do_in: 1.0, \
                Net.do_out: 1.0})
            y_pred_mu1 = sess.run(Net.output, feed_dict={\
                Net.x: train_data['x'],Net.v: train_data['v'], Net.t: 1-train_data['t']+train_data['t'], Net.s_mode1: 1., Net.s_mode2: 1., Net.do_in: 1.0, \
                Net.do_out: 1.0})
            ...
            y_pred_f_test = sess.run(Net.output, feed_dict={Net.x: test_data['x'], Net.v: test_data['v'], Net.t: test_data['t'], Net.s_mode1: 1., Net.s_mode2: 0.,Net.do_in: 1.0, Net.do_out: 1.0})
            y_pred_cf_test = sess.run(Net.output, feed_dict={Net.x: test_data['x'], Net.v: test_data['v'], Net.t: 1-test_data['t'], Net.s_mode1: 0., Net.s_mode2: 1.,Net.do_in: 1.0, Net.do_out: 1.0})
            y_pred_mu0_test = sess.run(Net.output, feed_dict={Net.x: test_data['x'],Net.v: test_data['v'], Net.t: test_data['t']-test_data['t'], Net.s_mode1: 0., Net.s_mode2: 0.,Net.do_in: 1.0, Net.do_out: 1.0})
            y_pred_mu1_test = sess.run(Net.output, feed_dict={Net.x: test_data['x'],Net.v: test_data['v'], Net.t: 1-test_data['t']+test_data['t'], Net.s_mode1: 1., Net.s_mode2: 1.,Net.do_in: 1.0, Net.do_out: 1.0})
...
def run(exp, args, dataDir, resultDir, train, val, test, device):
    ...
    v  = tf.placeholder("float", shape=[None, train['v'].shape[1]], name='v') # Outside IV
    ...
    dims = [train['x'].shape[1], FLAGS.dim_in, FLAGS.dim_out,train['v'].shape[1]]
    Net = VSDD(x, v, t, y_, p, s_mode1,s_mode2,FLAGS, r_alpha, r_beta, r_gamma,r_lambda,do_in, do_out, dims)
